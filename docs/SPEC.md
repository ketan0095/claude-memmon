...
**Rejected:** refusing and forgetting. **Why:** a blocked command that nobody re-runs is lost work. `record_block` dedupes on `(cmd, session_id)` (`:1804-1814`); `clear_pending` fires when the same session re-runs the same command and is allowed (`:1817-1824`); a notification fires once per sustained recovery — five unbroken HEALTHY samples, a command blocked within four hours still waiting, and a two-hour cooldown, all silenceable with `notify: never`. **Evidence:** [ASSERTED]. **Known incompleteness:** exact-string match only, so a session that re-runs the *scoped* variant the block message recommended never clears the original. Entries whose session has exited are pruned by the sampler (`_prune_pending`), which is exact and needed no threshold — it cleared 18 of 26 entries on the machine it was written for. There is still no TTL for a live session that has simply moved on; the log held one observed retry, 2.9 minutes after the block, which is too little evidence to pick one.
